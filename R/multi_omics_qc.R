...
multi_omics_qc <- function(multiassay) {
  ...
  mRNA <- df3[, 2]
  protein <- df3[, 3]
  min_cor <- cor(mRNA, protein, method = c("pearson")) #
  ...
  subacc <- c(subacc, rnaseqPCA = getLoadings(assays(subacc)[[1]], dolog = TRUE), mapFrom = 1L)
  subacc <- c(subacc, proteinPCA = getLoadings(assays(subacc)[[2]], center = FALSE, scale. = FALSE), mapFrom = 2L)
