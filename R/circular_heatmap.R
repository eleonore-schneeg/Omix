...
circular_heatmap<-function(network_communities,
    ...
    data="bundle_WGCNA_rna/rna-networkConstruction-auto_new.RData"){
  ...
  #pick top
  top=lapply(df0,function(x) {top_n(x,1, odds_ratio) })
  top2=lapply(top,function(x){x[1,]})
  top3=lapply(top2,function(x){x$description})
  top4=unlist(top3)
  mat0$process=top4
