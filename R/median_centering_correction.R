...
median_centering_correction <- function(matrix, batch_map, batch) {
  ...
  median_normalized_plot <- proBatch::matrix_to_long(
    median_normalized_matrix)
  median_normalized_plot$Batch <- batch_map[[paste(batch)]][
    match(median_normalized_plot$FullRunName, batch_map$FullRunName)]
  featurelevel_centering <- proBatch::center_feature_batch_medians_df(
    median_normalized_plot,
