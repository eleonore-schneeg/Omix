...
#' @param batch_correction Logical whether to perform  technical batch correction
...
process_protein <- function(multiassay,
  ...
  if (imputation == "distribution") {
    cli::cli_alert_success("Imputation of remaining missing values based on distribution")
  ...
  if (denoise == TRUE) {
    ...
    cli::cli_alert_success(paste("Using Limma on",covariates,'as biological confounders'))
  ...
  if (remove_feature_outliers == TRUE) {
    ...
    matrix <- matrix[idx, ]
    dim4=dim(matrix)[1]
    cli::cli_alert_success(paste(dim3-dim4,"feature outliers out of", dim3,"features detected and dropped"))
  ...
  if (remove_sample_outliers == TRUE) {
    ...
    dim6=dim(matrix)[2]
    cli::cli_alert_success(paste(dim6,"sample outliers out of", dim5," samples detected and dropped"))
  ...
  map_df <- map_df[match(colnames(matrix), map_df$colname), ][c("primary", "colname")]
...
processed_rna <- function(multiassay,
  ...
  rm.protein_processed <- !grepl("protein_processed", names(experiments(multiassay)))
