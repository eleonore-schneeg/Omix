...
integrative_results_supervised <- function(multiassay,
  ...
  if (enrichment_method == "enrichr") {
    pathways_detrimental <- lapply(detrimental, purrr::possibly(pathway_analysis_enrichr, otherwise=NA))
    pathways_protective <- lapply(protective, purrr::possibly(pathway_analysis_enrichr, otherwise=NA))
  }
