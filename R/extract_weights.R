...
extract_weigths <- function(model,
  ...
  if (!is.null(sense_check_variable)) {
    ...
    cor_p <- stats::cor(metadata[, sense_check_variable], t_p, use = "pairwise.complete.obs")
    protein_1$Correlation <- cor_p[match(protein_1$model_feature, colnames(cor_p))]

    t_r <- t(data.frame(model@data$mRNA))
    cor_r <- stats::cor(metadata[, sense_check_variable], t_r, use = "pairwise.complete.obs")
    rna_1$Correlation <- cor_r[match(rna_1$model_feature, colnames(cor_r))]
