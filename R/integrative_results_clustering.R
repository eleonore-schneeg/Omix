...
integrative_results_clustering <- function(multiassay,
  ...
  res1 <- clustering_DE_analysis(normalized_data=multimodal$rna_processed,
                                             colData=surv.info,
                                             dependent = "Cluster",
                                             levels = c('CS2','CS1'),
                                             log2FoldChange = 0.5 )
  ...
  res2 <- clustering_DE_analysis(normalized_data=multimodal$protein_processed,
                               colData=surv.info,
                               dependent = "Cluster",
                               levels = c('CS2','CS1'),
                               log2FoldChange = 0.2 )

  DE_res=list(rna=res1,
              proteins=res2)
  ...
  Up[[i]] <- list(rna=DE_res$rna$sig_feature[[i]]$up,
                     protein=DE_res$protein$sig_feature[[i]]$up)
  Down[[i]] <- list(rna=DE_res$rna$sig_feature[[i]]$down,
  ...
  Up_network[[i]] <- .multiomics_network_cluster(multiassay=multiassay,
  ...
  cluster= readr::parse_number(cluster[[1]][1])
  Down_network[[i]] <- .multiomics_network_cluster(multiassay=multiassay,
                                                     integration = "iCluster",
                                                     cluster=cluster,
                                             list = Down[[i]],
                                             correlation_threshold = correlation_threshold
