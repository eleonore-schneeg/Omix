...
get_metadata <- function(multiassay,
  ...
  if (omic == "protein") {
    ...
    map_protein=data.frame(sampleMap(multiassay))%>%
      filter(assay=="protein_processed")
  ...
  if (omic == "rna") {
    ...
    map_rna=data.frame(sampleMap(multiassay))%>%
      filter(assay=="rna_processed")
  }
  colData <- data.frame(SummarizedExperiment::colData(raw))
  if (omic == "protein") {
    order <- map_protein$primary[match(colnames(raw), map_protein$primary)]
  }

  if (omic == "rna") {
    order <- map_rna$primary[match(colnames(raw), map_rna$primary)]
  }
