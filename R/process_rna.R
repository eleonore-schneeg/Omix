...
#' @param min_sample Percentage of samples that have at least the `min_count`, or
#' the gene is filtered out
...
process_rna <- function(multiassay,
  ...
  if (filter == TRUE) {
    ...
    cli::cli_alert_success("Keeping only protein coding genes")
      ...
      cli::cli_alert_success(paste(dim1-sum(protein_coding),
                                   "/",dim1,'non protein coding genes were dropped'))
      cli::cli_alert_success(paste(sum(protein_coding),'protein coding genes kept for analysis'))
      dim1=as.numeric(dim(dds)[1])
    }
    cli::cli_alert_success(paste('QC parameters selected require genes to have at least',(min_sample *100),'% of samples with counts of',min_count,'or more'))
    ...
    dds <- dds[idx, ]
    cli::cli_alert_success(paste(dim1-keep,
    ...
    if (batch_correction == TRUE) {
      ...
      cli::cli_alert_success(paste("Using Limma on",batch,'to remove technical artefacts, and',covariates,'as biological confounders'))
    ...
    if (remove_sample_outliers == TRUE) {
      ...
      U <- pca$x
      outliers <- rownames(apply(U, 2,
                                 function(x) which(abs(x - mean(x)) >=
                                                     (3 * sd(x)))))
      matrix <- matrix[, colnames(matrix) %!in% outliers]
      cli::cli_alert_success(paste(length(outliers),"sample outliers out of", dim2," samples detected and dropped"))
    }
