...
#' @family Single-omic
#'
#' @importFrom MultiAssayExperiment MultiAssayExperiment listToMap colData metadata
...
protein_DE_analysis <- function(multiassay,
  ...
  if (isTRUE(denoised & isTRUE(cov == covariates))) {
    stop(cli::cli_alert_danger(
      paste("Processed protein data is already for designated covariates,
            model will be run for unadjusted covariates only")
    ))
    covariates <- setdiff(covariates, cov)
  }

  protein_raw <- MultiAssayExperiment::getWithColData(multiassay,
                                                      i = "protein_raw",
                                                      mode = "replace",
                                                      verbose = F)
  ...
  if (!is.numeric(colData[, dependent])) {
    ...
    contr.matrix <- limma::makeContrasts(contrasts = c(x, covariates),
  ...
  names(res) <- gsub(pattern = "-", replacement = "vs", x = names(res),
                     fixed = TRUE)
