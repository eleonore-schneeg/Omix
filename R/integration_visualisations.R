.multiomics_network<-function(multiassay,
  ...
  matrix <-  cbind(t(multimodal_object[[1]]),t(multimodal_object[[2]]))

  x=matrix[,colnames(matrix) %in% features_interest]
  ...
  a=apply(rho,1, function(x){.color.gradient(x)})
  ...
  mynode_labels=NULL
  mynode_labels=features_interest

  id_new_1=.get_ID_names(list[[1]],omic='rna',from='ensembl_gene_id',to='gene_name')
  id_new_2=.get_ID_names(list[[2]],omic='protein',from='uniprot_id',to='gene_name')
  mynode_labels=c(id_new_1,id_new_2)
