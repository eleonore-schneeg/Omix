...
library(utils)
outputDir <- tempdir()
rawdata_rna_fp <- file.path(outputDir, "raw_counts_MTG_SSC_CV.rds")
rawdata_protein_fp <- file.path(outputDir, "raw_proteomics_MTG_SSC_CV.rds")
map_rna_fp <- file.path(outputDir, "map_RNA_MTG_SSC_CV.rds")
map_prot_fp <- file.path(outputDir, "map_prot_MTG_SSC_CV.rds")
metadata_rna_fp <- file.path(outputDir, "metadata_rna_MTG_SSC_CV.rds")
metadata_prot_fp <-  file.path(outputDir, "metadata_prot_MTG_SSC_CV.rds")
individual_metadata_fp <-file.path(outputDir, "metadata_map_MTG_SSC_CV_new.rds")
ctd_fp <-file.path(outputDir, "ctd.rds")
ensembl_fp  <-file.path(outputDir, "ensembl.rds")
clusters_fp  <-file.path(outputDir, "clusters.rds")

download.file(url = "https://raw.githubusercontent.com/eleonore-schneeg/OmixData/main/data_MTG_SSC_CV/raw_counts_MTG_SSC_CV.rds",destfile=rawdata_rna_fp,headers = c(Authorization = paste("token", "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")))

download.file(url = "https://raw.githubusercontent.com/eleonore-schneeg/OmixData/main/data_MTG_SSC_CV/raw_proteomics_MTG_SSC_CV.rds",destfile=rawdata_protein_fp,headers = c(Authorization = paste("token", "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")))

download.file(url = "https://raw.githubusercontent.com/eleonore-schneeg/OmixData/main/data_MTG_SSC_CV/map_RNA_MTG_SSC_CV.rds",destfile=map_rna_fp ,headers = c(Authorization = paste("token", "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")))

download.file(url = "https://raw.githubusercontent.com/eleonore-schneeg/OmixData/main/data_MTG_SSC_CV/map_prot_MTG_SSC_CV.rds",destfile=map_prot_fp ,headers = c(Authorization = paste("token", "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")))

download.file(url = "https://raw.githubusercontent.com/eleonore-schneeg/OmixData/main/data_MTG_SSC_CV/metadata_rna_MTG_SSC_CV.rds",destfile=metadata_rna_fp ,headers = c(Authorization = paste("token", "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")))

download.file(url = "https://raw.githubusercontent.com/eleonore-schneeg/OmixData/main/data_MTG_SSC_CV/metadata_prot_MTG_SSC_CV.rds",destfile=metadata_prot_fp ,headers = c(Authorization = paste("token", "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")))

download.file(url = "https://raw.githubusercontent.com/eleonore-schneeg/OmixData/main/data_MTG_SSC_CV/metadata_map_MTG_SSC_CV_new.rds",destfile=individual_metadata_fp ,headers = c(Authorization = paste("token", "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")))
...
download.file(url = "https://raw.githubusercontent.com/eleonore-schneeg/OmixData/main/data_MTG_SSC_CV/clusters.rds",destfile=clusters_fp ,headers = c(Authorization = paste("token", "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")))



rawdata_rna <- as.data.frame(readRDS(paste0(outputDir,'/raw_counts_MTG_SSC_CV.rds')))
rawdata_protein <- as.data.frame(readRDS(paste0(outputDir,'/raw_proteomics_MTG_SSC_CV.rds')))
map_rna <- as.data.frame(readRDS(paste0(outputDir,'/map_RNA_MTG_SSC_CV.rds')))
map_prot <- as.data.frame(readRDS(paste0(outputDir,'/map_prot_MTG_SSC_CV.rds')))
metadata_rna<- as.data.frame(readRDS(paste0(outputDir,'/metadata_rna_MTG_SSC_CV.rds')))
metadata_prot <-  as.data.frame(readRDS(paste0(outputDir,'/metadata_prot_MTG_SSC_CV.rds')))
individual_metadata <- as.data.frame(readRDS(paste0(outputDir,'/metadata_map_MTG_SSC_CV_new.rds')))
...
## Pseudotemporal multi-omics integration using MOFA (MEIFESTO)


We need to create a new pseudo-temporal variable based on brain regions, 1 for MTG as it comes earlier in the disease trajectory, and 2 for SSC.

```{r , message = FALSE}
library(MultiAssayExperiment)
MultiAssayExperiment::colData(multiomics_object)$pseudotime=ifelse(colData(multiomics_object)$brain_region=='MTG',1,2)
```
...
```{r}
print(multiomics_object@metadata$integration$MEIFESTO)
```
...
```{r}
#model=readRDS('assets/model2.rds')
#multiomics_object@metadata$integration$MEIFESTO=model
```
...
library(ggpubr)
library(ggsignif)
integrated_object=multiomics_object@metadata$integration$MEIFESTO
```
...
suppressWarnings({MOFA2::correlate_factors_with_covariates(integrated_object, 
                                  covariates = c("AD","Braak",'pseudotime','MTG',
                                                 'SSC','amyloid','pTau','PHF1'), 
                                  plot="r")
...
Factors 6 and 1 will be chosen for downstream analysis:

-   Factor 6: Is strongly correlated with neuropathological variables including PHF1 and amyloid.

-   Factor 8: Is strongly correlated with pseudotemporal covariates, MTG and SSC
...
MOFA2::plot_factors(integrated_object,
             ...
             color_by='PHF1', shape_by = 'brain_region', scale = T)+
              xlab('Factor 6 ~ Pseudotime' )+
              ylab('Factor 1 ~ Neuropathology' )  
...
`slingshot` takes a matrix of embeddings in reduced dimension as input, which here are Factors 8 and 1 from the `MEIFESTO` multi-omics integration.
We can optionally specify the cluster to start or end the trajectory based on biological knowledge. Here, the starting cluster should be `3` as it coincides with lowest neuropathology levels and most "recent" affected region in terms of AD trajectory. 
...
clusters <- readRDS(paste0(outputDir,'/clusters.rds'))
...
```{r}
multiomics_object@metadata$integration$MEIFESTO=pseudotime$model
integrated_object=multiomics_object@metadata$integration$MEIFESTO
MOFA2::samples_metadata(integrated_object)$pseudotime
```
...
A negative weight indicates that the feature has higher levels in the samples with negative factor values, which in this analysis coincides with 
later Braak stages.
...
```{r}
intersect(weights$weights$ranked_weights_positive$rna,weights$weights$ranked_weights_positive$protein)
...
-   Community 1 is a mix of proteins and RNAs
-   Community 2 is a RNA only community
...
community_2=.community_graph(igraph=Weights_down_n$graph,
                            community_object=communities$community_object,
                            community=2)
...
```{r}
print(sort(community_2$hubs, decreasing = TRUE))
```


- Top gene in module 2 is `r names(sort(community_2$hubs, decreasing = TRUE)[1])`
- TREM2 is present here 
- From prior knowledge, many of the genes in module 2 are related to microglial activation
...
- Gene set enrichment analysis is a method to infer biological pathway activity from gene expression data.
- Here we use “pre-ranked GSEA” which takes as input a gene list sorted by their weight in the Factor of interest post integration, here factor 6.
- Down-regulated pathways are up-regulated in samples with negative factor values, here late Braak stages samples
- Up-regulated pathways are strong in samples with positive factor values, here control and early braak stages 

### RNA level enrichment

```{r}
gene_list = weights$weights_df$rna$Weights
names(gene_list) =  weights$weights_df$rna$Feature
gene_list = sort(gene_list, decreasing = TRUE)
gene_list = gene_list[!duplicated(names(gene_list))]


rna_enrichment=GSEA(gene_list=gene_list,
          pval=0.05)
...
```{r}
gene_list = weights$weights_df$protein$Weights
names(gene_list) =  weights$weights_df$protein$Feature
gene_list = sort(gene_list, decreasing = TRUE)
gene_list = gene_list[!duplicated(names(gene_list))]


protein_enrichment=GSEA(gene_list=gene_list,
          pval=0.05)

protein_enrichment$Plot
```
...
- Here we related each module eigen value to the pseudotime inferred from MOFA integration emmbeddigns (Factors 6/8)

```{r, message= FALSE}

pseudotime_ordering_modules=pseudotime_ordering_modules(metadata=modules$metadata_modules,
                                       modules=colnames(modules$modules_eigen_value),
                                       variable='PHF1',
...
gb=ggpubr::ggscatter(modules$metadata_modules, y = "PHF1", x = "pseudotime", palette = "jco",
...
gg=ggpubr::ggscatter(modules$metadata_modules, y = "amyloid", x = "Braak_num", palette = "jco",
                     ...
                     cor.coef = TRUE, # Add correlation coefficient. see ?stat_cor
                     cor.coeff.args = list(method = "pearson", label.sep = "\n"))
gb=ggpubr::ggscatter(modules$metadata_modules, y = "amyloid", x = "pseudotime", palette = "jco",
                     add = "reg.line", conf.int = TRUE,   add.params = list(color = "blue", fill = "lightgray"), # Customize reg. line
                     cor.coef = TRUE, # Add correlation coefficient. see ?stat_cor
                     cor.coeff.args = list(method = "pearson", label.x = 4, label.sep = "\n"))
plot(gg)
...
gg=ggpubr::ggscatter(modules$metadata_modules, x = "pseudotime", y = paste(i),
          color = "diagnosis", palette = "jco",
          add = "reg.line", conf.int = TRUE)+stat_cor(aes(color = diagnosis), label.x = 1)
plot(gg)
}
```
...
for(i in colnames(modules$modules_eigen_value)){
gg=ggpubr::ggscatter(modules$metadata_modules, x = "PHF1", y = paste(i), palette = "jco",
                     add = "reg.line", conf.int = TRUE,   add.params = list(color = "blue", fill = "lightgray"), # Customize reg. line
                     cor.coef = TRUE, # Add correlation coefficient. see ?stat_cor
...
for(i in colnames(modules$modules_eigen_value)){
gg=ggpubr::ggboxplot(modules$metadata_modules, x = "diagnosis", y = paste(i),add='jitter', color="diagnosis")+
  xlab('Diagnosis')+ylab(paste('Eigen value in',i))+theme_classic()+
  geom_signif(comparisons = list(c('AD', 'Control')))+
  scale_color_manual(values = c("#2E9FDF","#E7B800"))+facet_wrap(~brain_region)
plot(gg)
}
```
...
for(i in colnames(modules$modules_eigen_value)){
gg=ggpubr::ggboxplot(modules$metadata_modules, x = "Braak", y = paste(i),add='jitter', color="Braak")+
  xlab('Diagnosis')+ylab(paste('Eigen value in',i))+theme_classic()+facet_wrap(~brain_region)
plot(gg)
}
```
...
for(i in colnames(modules$modules_eigen_value)){
gg=ggpubr::ggboxplot(modules$metadata_modules, x = "brain_region", y = paste(i),add='jitter',color='brain_region')+
  xlab('Diagnosis')+ylab(paste('Eigen value in',i))+theme_classic()+
  geom_signif(comparisons = list(c('MTG', 'SSC')))+
  scale_color_manual(values = c("#DC0000FF","forestgreen"))
plot(gg)
}
```
