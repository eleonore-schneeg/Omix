...
download.file(url = "https://raw.githubusercontent.com/eleonore-schneeg/OmixData/main/data_MTG_SSC_CV/raw_counts_MTG_SSC_CV.rds",destfile=rawdata_rna_fp,headers = c(Authorization = paste("token", "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")))

download.file(url = "https://raw.githubusercontent.com/eleonore-schneeg/OmixData/main/data_MTG_SSC_CV/raw_proteomics_MTG_SSC_CV.rds",destfile=rawdata_protein_fp,headers = c(Authorization = paste("token", "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")))

download.file(url = "https://raw.githubusercontent.com/eleonore-schneeg/OmixData/main/data_MTG_SSC_CV/map_RNA_MTG_SSC_CV.rds",destfile=map_rna_fp ,headers = c(Authorization = paste("token", "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")))

download.file(url = "https://raw.githubusercontent.com/eleonore-schneeg/OmixData/main/data_MTG_SSC_CV/map_prot_MTG_SSC_CV.rds",destfile=map_prot_fp ,headers = c(Authorization = paste("token", "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")))

download.file(url = "https://raw.githubusercontent.com/eleonore-schneeg/OmixData/main/data_MTG_SSC_CV/metadata_rna_MTG_SSC_CV.rds",destfile=metadata_rna_fp ,headers = c(Authorization = paste("token", "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")))

download.file(url = "https://raw.githubusercontent.com/eleonore-schneeg/OmixData/main/data_MTG_SSC_CV/metadata_prot_MTG_SSC_CV.rds",destfile=metadata_prot_fp ,headers = c(Authorization = paste("token", "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")))

download.file(url = "https://raw.githubusercontent.com/eleonore-schneeg/OmixData/main/data_MTG_SSC_CV/metadata_map_MTG_SSC_CV_new.rds",destfile=individual_metadata_fp ,headers = c(Authorization = paste("token", "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")))
...
download.file(url = "https://raw.githubusercontent.com/eleonore-schneeg/OmixData/main/data_MTG_SSC_CV/clusters.rds",destfile=clusters_fp ,headers = c(Authorization = paste("token", "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")))
...
```{r , message = FALSE}
MultiAssayExperiment::colData(multiomics_object)$pseudotime=
  ifelse(MultiAssayExperiment::colData(multiomics_object)$brain_region=='MTG',1,2)
```
...
Since negative weights coincide wiht negative factor values in this case, all down-regulated pathways are those in fact up-regulated in AD. 
...
Complement activation is up-regulated both at the RNA and protein level.

```{r}
complement_rna=rna_enrichment[["Results"]]$leadingEdge[which(rna_enrichment[["Results"]]$pathway=='complement activation')]
protein_enrichment[["Results"]]$leadingEdge[which( protein_enrichment[["Results"]]$pathway=='complement activation')]

```


If users have a chatGPT personal access token, it is possible to query chatGPT with prompts like the following:

```{r}

ans=ask_chatgpt(prompt="Briefly explain what 'complement acitvation' corresponds to in terms of biological pathways, and how it has been related to Alzheimer's",
                  api_key="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")
cat(ans)

```

```{r}
for (i in unlist(complement_rna)){
  ans=ask_chatgpt(prompt=paste("Explain in 3 lines maximum the function of the following protein:",i),
                  api_key="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")
  cat("\nProtein",paste(i),':',ans)
}
...
gg=ggpubr::ggboxplot(modules$metadata_modules, 
  ...
  ylab(paste('Eigen value in',i))+
  theme_classic()+
  ggsignif::geom_signif(comparisons = list(c('MTG', 'SSC')))+
